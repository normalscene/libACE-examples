...
{
    eMtErr eRet = emt_is_valid_pack(pinBuf, usLen);
...
*****************************************************************************/
void vCmSetEC(uint8_t ucEC1, uint8_t ucEC2)
{
...
{
    eMtErr eRet = emt_find_valid_pack(pinBuf, usLen,  pusFirstOff,  pusFirstLen);
    return eRet;
}
...
eCmErr ecm_3761_pack(sCmPacket* psCmPacket, uint8_t * pBuf, uint16_t *pusBufLen)
{
    ...
    for(i = 0; i < ucCmdNum; i++)
    {
        ...
        eRet = (eMtErr)emtTransAppData_cm_2_mt(emtCmd, &(psCmPacket->sCmdData[i].uAppData), &(psPack->sData[i].uApp));
...
eCmErr ecm_3761_unpack(uint8_t * pBuf, uint16_t usBufLen, sCmPacket* psCmPacket)
{
    ...
    for(i = 0; i < psCmPacket->ucCmdNum; i++)
    {
        ...
        eRet = (eMtErr)emtTransAppData_mt_2_cm(psmtPack->sData[i].eCmd, 
...
*****************************************************************************/
int32_t ncm_3761_checkAndGetPacket(uint8_t  *pinBuf, uint16_t usLen, uint16_t *pusFirstOff, uint16_t *pusFirstLen)
{
    eCmErr eRet  = MT_OK;
