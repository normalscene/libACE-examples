...
uint8_t g_ucPackMem[PACK_MEM_SIZE] = {0}; // 测试封装命令的参数的内存空间
uint8_t g_ucOutBuf[OUT_BUF_LEN]    = {0}; // 存放输出报文
...
void show_sub_name(const char * str)
{
    fm_print(str,FR_YELLOW|BK_NORMAL);
}

void show_item_value(const char *str)
{
...
void printf_buffer_color(const char * pBuf, uint16_t usBufLen)
{
    ...
    {       
        ...
        {   
            memset(buf, 0,10);
            sprintf(buf,"%02X ", (uint8_t)pBuf[i]);
            fm_print(buf,FR_WHITE|BK_MAGENTA|FM_BOLD);
        }
...
*****************************************************************************/
void print_buffer(const uint8_t * pBuf, uint16_t* pusBufLen)
{
...
eMtErr test_pack_0001_m2s()
{
   ...
   smtPack *pscmPacket = (smtPack*)g_ucPackMem;
    ...
    /* 4 调用函数 */
    err = emt_pack(pscmPacket, &usBuflen, (uint8_t*)g_ucOutBuf);
    if(err != MT_OK)
    {
...
eMtErr test_pack_0001()
{
    ...
    // ctrl_filed
    err = eMtGetCtrl(psPack->eMainCmd, eDir, psPack->sCtrl.eRPM, &(psPack->sCtrl));
    ...
    /* 4 调用函数 */ 
    err = emt_pack(psPack, &usBuflen, (uint8_t *)g_ucOutBuf);
    if(err != MT_OK)
    {
...
eMtErr test_pack_0002()
{
    ...
    psPack->sCtrl.eRPM = MT_PRM_PASIVE;
    // ctrl_filed
    err = eMtGetCtrl(psPack->eMainCmd, eDir, MT_PRM_ACTIVE, &(psPack->sCtrl));
...
eMtErr test_pack_0103()
{
    ...
    psPack->sCtrl.eRPM = MT_PRM_ACTIVE;
    // ctrl_filed
    err = eMtGetCtrl(psPack->eMainCmd, eDir,MT_PRM_PASIVE, &(psPack->sCtrl));
...
eMtErr test_pack_0421()
{
   ...
   sCmPacket *pscmPacket = (sCmPacket*)g_ucPackMem;
    ...
    /* 4 调用函数 */
    err = ecm_3761_pack(pscmPacket, (uint8_t*)g_ucOutBuf, &usBuflen);
    if(err != MT_OK)
    {
        printf("ecm_3761_pack error %d\n", err);
        return;
    ...
    switch(emtCmd)
    {
        case CMD_AFN_0_F3_ONE_BY_ONE:
        {
            ...
            for(i = 0; i < psOnebyOne->ucNum; i++)
            {
                ...
                err = emt_get_cmd_info(psOnebyOne->sOne[i].eCmd, MT_DIR_S2M, &sCmdInfo);
                ...
                {
                    eRet2 = emt_get_cmd_info(psOnebyOne->sOne[i].eCmd, MT_DIR_M2S, &sCmdInfo);
                    if(eRet2 == MT_OK)
                    {
                        pStr =  sCmdInfo.pName; 
                    }
        ...
        case CMD_AFN_4_F38_CFG_ASK_1:
        {
            ...
            for(i = 0; i < pData->ucM; i++)
            {
               ...
               for(j = 0; j < pData->sOne[i].ucFN; j++)
               {
                  ...
                  err = emt_get_cmd_info(eCmd, MT_DIR_M2S, &sCmdInfo);
                  if(err == MT_OK)
                  {
                     printf("%s\n", sCmdInfo.pName);
                  }
        ...
        case CMD_AFN_9_F3_TML_OTHER_CFG:
            ...
            {    
                sMtCmdInfor    sCmdInfo = {CMD_AFN_F_UNKOWN};
                ...
                for(i = 0; i < pData->ucNum; i++)
                {
                    ...
                    err = emt_get_cmd_info(pData->eCfgCmd[i], MT_DIR_M2S, &sCmdInfo);  
                    if(MT_OK == err)
                    {
                        printf("%s\n", sCmdInfo.pName);
                    }
...
void show_app_data_adp(eMtDir eDir, sCmSub *pSub)
{
    ...
    sMtCmdInfor    sCmdInfor   = {CMD_AFN_F_UNKOWN};
    eMtErr         err        = MT_OK;    
   // pMtFunc        pFunc       = NULL;
    char *         pStr        = NULL;
    eMtCmd         emtCmd  ;
   ...
   show_sub_name("子命令ID\t");
   err = emt_get_cmd_info(emtCmd, eDir, &sCmdInfor);
   if(err != MT_OK)
   {
        show_error("错误的命令类型!");
        return;
...
void show_app_data(eMtDir eDir, sMtData *pSub)
{
   ...
   show_sub_name("子命令ID\t");
   err = emt_get_cmd_info(pSub->eCmd, eDir, &sCmdInfor);
   if(err != MT_OK)
   {
        show_error("错误的命令类型!");
        return;
...
void test_unpack_adp(int nstart, int argc, char *argv[])
{
   ...
   char OutBuf[1024] = {0};
    ...
    //调用解析函数
    eMtErr err = ecm_3761_unpack(pInBuf, usLen, psUnpack);
...
void unpack_analyse(int nstart, int argc, char *argv[])
{
    ...
    // 
    err = emt_unpack_common(&sUnpackCommon,pInBuf, usLen);
    if(MT_OK != err)
    {
    ...
    // FIR
    uint8_t ucFIR = (ucTemp & 0x40) ? 1:0;
    show_sub_name("\t\tFIR:\t\t");
    printf("%d\n",  ucFIR);

    uint8_t ucFIN = (ucTemp & 0x40) ? 1:0;
    show_sub_name("\t\tFIN:\t\t");
    printf("%d\t", ucFIN);
    ...
    sMtPnFn sPnFn;
    err =  emt_dadt_to_pnfn(&sDaDt, &sPnFn);
    sMtCmdPn sCmdPn[64];
    ...
    {             
        err = emt_pnfn_to_cmdpn(eAfn, &sPnFn, 1,   (sMtCmdPn*)&sCmdPn, &ucNumCmdPn);
        ...
        for(i = 0; i < ucNumCmdPn; i++)
        {
            ...
            printf("%d\n",  sCmdPn[i].usPn);
            show_sub_name("\t\tcmd:\t\t");
            printf("%04X\t",  (uint32_t)sCmdPn[i].eCmd);
            err = emt_get_cmd_info(sCmdPn[i].eCmd, eDir, &sCmdInfor);
            if(MT_OK != err)
            {
...
void test_unpack(int nstart, int argc, char *argv[])
{
    ...
    //调用解析函数
    eMtErr err = emt_unpack(psUnpack, pInBuf, usLen);
...
int main(int argc, char *argv[])
{
    int opt = 0;
    int longIndex = 0;
    char *strOpt = (char*)"uohcnmap:t:e:a:d:";
    int    iTest= 0;// 测试例ID
